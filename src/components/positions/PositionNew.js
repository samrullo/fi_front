...
const PositionNew = () => {
  ...
  useEffect(() => {
    const loadSecurities = async () => {
      try {
        ...
        setSecurityOptions(data.map((bond) => ({
          value: bond.id,
          label: `${bond.asset_name} (${bond.identifier_client})`
        })));
        if (data.length) setSecurityId(data[0].id);
      } catch (err) {
        alert("Failed to fetch bonds");
      }
    };
    loadSecurities();
  }, []);
  ...
  return <GenericNewData title="New Position" formFields={formFields} handleNewData={handleNewData} />;
};
